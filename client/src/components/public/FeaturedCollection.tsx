...
const FeaturedCollection = ({ }: FeaturedCollectionProps) => {
    ...
    const [inquiryModal, setInquiryModal] = useState<{ open: boolean; product: any }>({ open: false, product: null });
    ...
    useEffect(() => {
        ...
        if (selectedCategory !== 'all') {
            filtered = allProducts.filter(p => p.category?._id === selectedCategory);
        }
                    ...
                    <>
                        ...
                        <div className="flex flex-wrap justify-center gap-3 mb-12">
                            <button
                                ...
                                className={`px-5 py-2.5 border rounded-full text-md font-normal  tracking-wider transition-all duration-200 ${selectedCategory === 'all'
                            ...
                            <div className="gap-3 flex flex-wrap justify-center ">
                                {categories.map((category) => (
                                    <button
                                        ...
                                        className={`px-5 py-2.5 border rounded-full text-md font-normal tracking-wider transition-all duration-200 ${selectedCategory === category._id
                                            ? 'bg-[#BFD0E3] border-black text-black border border-white shadow-lg'
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                    {displayedProducts.map((product, index) => (
                                        <ProductCard
                                            ...
                                            product={product}
                                            index={index}
                                            onInquire={(prod) => setInquiryModal({ open: true, product: prod })}
                                        />
                                    ))}
                                </div>
                                        ...
                                        <button
                                            onClick={() => {
                                                const categorySlug = selectedCategory === 'all'
                                                    ? 'all'
                                                    : categories.find(c => c._id === selectedCategory)?.slug;
